...
public class EduTeacherController {
    ...
    public Msg pageTeacher(
        ...
        return Msg.ok().data("total", total).data("rows", records);
    }

    @ApiOperation(value = "带条件的分页查询讲师")
    @ResponseBody
    @PostMapping(value = "/pageTeacherCondition/{page}/{limit}")
    public Msg pageTeacherCondition(
            @ApiParam(name = "page", value = "当前页码", required = true)
            @PathVariable Long page,
            @ApiParam(name = "limit", value = "每页记录数", required = true)
            @PathVariable Long limit,
    ...
    public Msg getTeacherById(
            @ApiParam(name = "id", value = "讲师ID", required = true)
            @PathVariable String id) {

        EduTeacher teacher = eduTeacherService.getById(id);
        return Msg.ok().data("item", teacher);
    }
